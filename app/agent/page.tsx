...
> = {
  r32: {
    label: "32强",
    color: "border-zinc-600/50 bg-zinc-800/30 text-zinc-300",
    icon: "⚔️",
  },
  r16: {
    label: "16强",
    color: "border-blue-500/30 bg-blue-900/20 text-blue-300",
    icon: "⚔️",
  },
  qf: {
    label: "四分之一决赛",
    color: "border-violet-500/30 bg-violet-900/20 text-violet-300",
    icon: "🔥",
  },
  sf: {
    label: "半决赛",
    color: "border-amber-500/30 bg-amber-900/20 text-amber-300",
    icon: "🏟️",
  },
  final: {
    label: "决赛",
    color: "border-pitch/50 bg-pitch/10 text-pitch-bright",
    icon: "🏆",
  },
};
      ...
      {finalPrediction && (
        <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-3 flex flex-col items-center justify-center text-center relative overflow-hidden">
          ...
          <div className="relative">
            ...
            <div className="text-3xl font-mono font-black text-amber-400 my-2 tabular-nums">
      ...
      {darkHorses.length > 0 && (
        <div className="rounded-xl border border-warn/20 bg-warn/5 p-3">
          ...
          {darkHorses.slice(0, 3).map((d, i) => (
            <div
              key={d.team}
              className={`py-2 ${i < darkHorses.slice(0, 3).length - 1 ? "border-b border-warn/10" : ""}`}
            >
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-[10px] font-mono text-warn">
                  {(d.probability * 100).toFixed(1)}% 夺冠
                </span>
              </div>
            </div>
          ))}
  ...
  return (
    <div className="mt-3 rounded-xl border border-border bg-surface/60 overflow-hidden">
      <div className="px-4 py-2.5 border-b border-border flex items-center justify-between">
        <span className="text-xs font-bold text-pitch-bright">
          🏆 冠军晋级路径
        </span>
        <Link
          href="/bracket"
      ...
      {/* 横向时间轴 */}
      <div
        className="p-3 flex gap-1.5 overflow-x-auto"
        ...
        {stages.map((stage, si) => {
          ...
          return (
            <div key={stage} className="flex items-center gap-1.5 shrink-0">
              <div
                className={`rounded-lg border px-3 py-2 min-w-[120px] ${meta.color}`}
              >
                ...
                {grouped[stage].map((m, i) => (
                  <div key={i} className="text-[10px] leading-5">
                    ...
                    <div
                      className={`text-[9px] mt-0.5 ${m.state === "finished" ? "text-pitch-bright" : "text-violet-400"}`}
                    >
...
function SentimentPanel({ snap }: { snap: SentimentSnapshot }) {
  ...
  // 因子整体倾向颜色
  const factorColor = (net: number) =>
    net > 0 ? { ring: "border-pitch-bright/25", bg: "bg-pitch-bright/8", label: "text-pitch-bright", bar: "bg-pitch-bright/60" }
    : net < 0 ? { ring: "border-red-400/25", bg: "bg-red-500/8", label: "text-red-400", bar: "bg-red-400/60" }
    : { ring: "border-border/30", bg: "bg-surface-2/40", label: "text-muted", bar: "bg-muted/40" };
  ...
  return (
    <div className="mt-4 rounded-2xl overflow-hidden border border-violet-500/25 bg-linear-to-br from-violet-950/60 via-violet-900/20 to-transparent">
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-violet-500/10 border-b border-violet-500/20">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[11px] px-2 py-0.5 rounded-full bg-violet-500/20 text-violet-300 font-mono">{snap.total} 条数据</span>
          <a href="/data" className="text-[10px] px-2 py-1 rounded-lg border border-violet-500/30 text-violet-400 hover:bg-violet-500/10 transition-colors">查看详情 →</a>
        </div>
      </div>
      ...
      <div className="px-4 py-3 space-y-4">
        ...
        {factors.length > 0 && (
          <div>
            <div className="text-[10px] text-muted/60 uppercase tracking-wider mb-2">🔬 舆情影响因子</div>
            <div className="grid grid-cols-2 gap-2">
              {factors.map((f) => {
                const total = f.pos + f.neg + f.neu || 1;
                const posW = Math.round((f.pos / total) * 100);
                const negW = Math.round((f.neg / total) * 100);
                const c = factorColor(f.net);
                return (
                  <div key={f.category} className={`rounded-xl border p-3 ${c.ring} ${c.bg}`}>
                    {/* 因子标题 */}
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className={`text-[11px] font-mono font-bold ${c.label}`}>
                        {f.net > 0 ? "+" : ""}{f.net}
                      </span>
                    </div>
                    {/* 声量三色条 */}
                    <div className="flex h-1.5 rounded-full overflow-hidden bg-surface-2/50 mb-2 gap-px">
                      <div className="bg-pitch-bright/60" style={{ width: `${posW}%` }} />
                      <div className="bg-surface-2/70" style={{ width: `${100 - posW - negW}%` }} />
                      <div className="bg-red-400/55" style={{ width: `${negW}%` }} />
                    </div>
                    {/* 最受益 / 最受损球队 */}
                    <div className="space-y-0.5">
                      {f.topTeam !== "—" && f.topTeamNet !== 0 && (
                        <div className="flex items-center gap-1 text-[9px]">
                          <span className="text-pitch-bright/70">↑</span>
                          <span className="text-muted/70 truncate">{f.topTeam}</span>
                          <span className="text-pitch-bright/80 font-mono ml-auto">+{f.topTeamNet}</span>
                        </div>
                      )}
                      {f.worstTeam !== "—" && f.worstTeamNet < 0 && f.worstTeam !== f.topTeam && (
                        <div className="flex items-center gap-1 text-[9px]">
                          <span className="text-red-400/70">↓</span>
                          <span className="text-muted/70 truncate">{f.worstTeam}</span>
                          <span className="text-red-400/80 font-mono ml-auto">{f.worstTeamNet}</span>
                        </div>
                      )}
                    </div>
                  </div>
        ...
        {/* ── 球队声量分布 ── */}
        <div>
          <div className="text-[10px] text-muted/60 uppercase tracking-wider mb-2">📊 球队声量分布</div>
            ...
            {snap.teams.slice(0, 6).map((t) => {
              ...
              return (
                <div key={t.teamId} className="flex items-center gap-2">
                  <span className={`text-[10px] shrink-0 ${isPos ? "text-pitch-bright/80" : t.net < 0 ? "text-red-400/80" : "text-muted/70"}`}>
                    {isPos ? "📈" : t.net < 0 ? "📉" : "➡️"}
                  </span>
                  <span className="text-[10px] text-muted/70 w-12 truncate shrink-0">{t.teamId}</span>
                  <div className="flex-1 flex h-2 rounded-full overflow-hidden bg-surface-2/40 gap-px">
                    <div className="bg-pitch-bright/55" style={{ width: `${posW}%` }} />
                    <div className="bg-surface-2/80" style={{ width: `${100 - posW - negW}%` }} />
                    <div className="bg-red-400/50" style={{ width: `${negW}%` }} />
                  </div>
                  ...
                  {/* 净值进度 */}
                  <span className={`text-[9px] font-mono w-6 text-right shrink-0 ${isPos ? "text-pitch-bright" : t.net < 0 ? "text-red-400" : "text-muted"}`}>
                    {isPos ? "+" : ""}{t.net}
                  </span>
                </div>
              );
        ...
        {snap.generalNotes.length > 0 && (
          <div className="rounded-lg bg-surface-2/30 border border-border/30 px-3 py-2">
            ...
            <div className="text-[10px] text-muted/70 leading-relaxed">{snap.generalNotes.slice(0, 2).join("；")}</div>
          </div>
        )}

        <div className="flex items-center justify-between text-[9px] text-muted/40">
          <span>📊 舆情因子已融入模拟修正系数（±15% 区间）</span>
...
function ReasoningChain({ steps }: { steps: string[] }) {
  return (
    <div className="mt-3 rounded-xl border border-data/20 bg-data/5 p-3">
      <div className="text-[10px] text-data-bright uppercase tracking-wider mb-2">
        🔗 推理链路
      </div>
      <div className="flex flex-wrap gap-2">
        {steps.map((step, i) => (
          ...
          >
            <span className="w-4 h-4 rounded-full bg-data/20 text-data-bright flex items-center justify-center text-[9px] shrink-0 font-bold">
              {i + 1}
            </span>
...
function ExploreModules() {
  return (
    <div className="mt-4">
      <div className="text-[10px] text-muted uppercase tracking-wider mb-2 px-1">
        🔍 深入探索
      </div>
          ...
          >
            <div className="flex items-start justify-between mb-1.5">
              ...
              <div className="flex items-center gap-1">
                {m.badge && (
                  <span className="text-[8px] px-1 py-0.5 rounded-full bg-rose-500/80 text-white font-bold animate-pulse">
                    {m.badge}
                  </span>
                )}
            ...
            <div className="text-xs font-semibold text-foreground group-hover:text-foreground transition-colors">
              {m.title}
            </div>
      ...
      {RUN_STAGES.map((s, i) => {
        ...
        return (
          <div key={s.key} className="flex items-center gap-1">
            <div
              className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-medium transition-all duration-500 ${
                done
                  ? "bg-pitch/20 text-pitch-bright border border-pitch/20"
                  : active
                    ? "bg-data/20 text-data-bright border border-data/30 shadow-[0_0_8px_rgba(14,165,233,0.3)]"
            ...
            {i < RUN_STAGES.length - 1 && (
              <span
                className={`text-[10px] transition-colors duration-500 ${done ? "text-pitch/60" : "text-border"}`}
              >
                ›
              </span>
            )}
...
export default function AgentPage() {
  ...
  const drainQueue = () => {
    ...
    setTasks((prev) =>
      prev.map((t) => (t.id === next.id ? { ...t, status: "running" } : t)),
    );
  ...
  const triggerChat = async (
    ...
    const placeholder: AgentChatMessage = {
      ...
      content: "",
      streaming: true,
      currentPhase: isFirstRun ? "think" : undefined,
    };
    ...
    try {
      ...
      if (isFirstRun && meta?.topChampions) {
        const top = meta.topChampions as {
          team: string;
          probability: number;
          teamId?: string;
        }[];
        ...
        if (meta.detailedResult) {
          useAppStore.setState({
            detailedResult:
              meta.detailedResult as import("@/lib/prediction/detailedSim").DetailedSimResult,
  ...
  const sendMessage = (text?: string) => {
    ...
    if (!loadingRef.current) {
      ...
      setTasks((prev) => [
        ...prev,
        { id, item: { type: "text", content }, status: "running" },
      ]);
      ...
      setTasks((prev) => [
        ...prev,
        { id, item: { type: "text", content }, status: "pending" },
      ]);
    }
  };
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-56px)] lg:h-screen max-h-screen">
      ...
      <div className="shrink-0 px-5 py-3 border-b border-border bg-surface/80 backdrop-blur-xl flex items-center gap-3">
        <div className="w-8 h-8 rounded-xl bg-linear-to-br from-pitch-bright to-data flex items-center justify-center text-sm">
          ⚽
        </div>
        <div>
          ...
          <div className="text-[10px] text-muted">
            本页回答：问我任何问题 / 一键生成完整预测报告 · Qwen 驱动
          </div>
        </div>
        <div className="ml-auto flex items-center gap-4">
          <Link
            href="/bracket"
            className="hidden sm:flex items-center gap-1 text-[11px] text-data hover:text-data-bright transition-colors"
          >
            <svg
              width="11"
              height="11"
      ...
      <div className="flex-1 overflow-y-auto px-4 py-5 space-y-5">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center gap-4">
            {loading ? (
              ...
              <>
                <div className="relative">
                  <div className="w-20 h-20 rounded-3xl bg-linear-to-br from-pitch-bright/20 to-data/20 border border-pitch-bright/20 flex items-center justify-center text-4xl">
                    ⚽
                  </div>
                  {/* 旋转光环 */}
                  <div className="absolute inset-0 rounded-3xl border-2 border-transparent border-t-pitch-bright/60 animate-spin" />
                </div>
                <div>
                ...
                <div className="flex flex-col gap-2 w-56">
                  {[
                    { icon: "🌐", label: "赛程数据采集", delay: "0ms" },
                    {
                      icon: "⚙️",
                      label: "3000 次蒙特卡洛模拟",
                      delay: "400ms",
                    },
                    { icon: "🧠", label: "Qwen AI 深度分析", delay: "800ms" },
                  ].map((step) => (
                    <div
                      key={step.label}
                      className="flex items-center gap-2.5 px-3 py-2 rounded-xl bg-surface-2/60 border border-border/40"
                    ...
                    >
                      ...
                      <span className="flex gap-0.5 shrink-0">
                        <span
                          className="w-1 h-1 rounded-full bg-pitch-bright animate-bounce"
                          style={{ animationDelay: step.delay }}
                        />
                        <span
                          className="w-1 h-1 rounded-full bg-pitch-bright animate-bounce"
                          style={{
                            animationDelay: `calc(${step.delay} + 150ms)`,
                          }}
                        />
                        <span
                          className="w-1 h-1 rounded-full bg-pitch-bright animate-bounce"
                          style={{
                            animationDelay: `calc(${step.delay} + 300ms)`,
                          }}
                        />
                      </span>
                    </div>
                ...
                {/* 骨架卡片预览 */}
                <div className="w-full max-w-sm mt-2 space-y-2 opacity-40">
                  <div className="h-3 bg-surface-2 rounded-full animate-pulse w-3/4 mx-auto" />
                  <div className="h-3 bg-surface-2 rounded-full animate-pulse w-full" />
                  <div className="h-3 bg-surface-2 rounded-full animate-pulse w-5/6 mx-auto" />
                  <div className="mt-3 grid grid-cols-3 gap-2">
                    {[1, 2, 3].map((i) => (
                      <div
                        key={i}
                        className="h-12 bg-surface-2 rounded-xl animate-pulse"
              ...
              <>
                ...
                <div className="flex gap-1.5 mt-1">
                  {["Elo", "泊松", "蒙特卡洛", "Qwen AI"].map((t) => (
                    <span
                      key={t}
                      className="text-[10px] px-2 py-0.5 rounded-full bg-surface-2 border border-border text-muted"
                    >
                      {t}
                    </span>
                  ))}
        ...
        {messages.map((msg, idx) => (
          ...
          >
            {msg.role === "assistant" && (msg.content || !msg.streaming) && (
              <div className="w-8 h-8 rounded-xl bg-linear-to-br from-pitch-bright to-data flex items-center justify-center text-sm shrink-0 mt-1">
                ⚽
              </div>
            )}

            <div
              className={`${msg.role === "user" ? "max-w-[72%] order-first" : "max-w-[88%] w-full"}`}
            >
                ...
                <div className="mt-1 space-y-0">
                  {msg.meta.topChampions && (
                    <StatsPanel
                      ...
                      finalPrediction={
                        msg.meta.finalPrediction as FinalPrediction
                      }
                      darkHorses={(msg.meta.darkHorses ?? []) as DarkHorse[]}
                    />
                  )}
                  {msg.meta.championPath &&
                    (msg.meta.championPath as ChampionPathItem[]).length >
                      0 && (
                      <ChampionPathCard
                        path={msg.meta.championPath as ChampionPathItem[]}
                      />
                    )}
                  {msg.meta.reasoningChain &&
                    (msg.meta.reasoningChain as string[]).length > 0 && (
                      <ReasoningChain
                        steps={msg.meta.reasoningChain as string[]}
                      />
                    )}
                  {msg.meta.sentimentSnapshot &&
                    (msg.meta.sentimentSnapshot as SentimentSnapshot).teams
                      .length > 0 && (
                      <SentimentPanel
                        snap={msg.meta.sentimentSnapshot as SentimentSnapshot}
                      />
                    )}
                  {/* 探索更多功能入口 */}
                  <ExploreModules />
              ...
              {msg.role === "assistant" &&
                ...
                !msg.meta?.topChampions && (
                  <div className="mt-1 text-[10px] text-muted px-1">
                    {msg.meta.model} · Qwen AI
                  </div>
                )}
            </div>

            {msg.role === "user" && (
              <div className="w-8 h-8 rounded-xl bg-surface-2 border border-border flex items-center justify-center text-sm shrink-0 mt-1">
                👤
              </div>
            )}
          </div>
        ...
        {/* 多轮对话 loading 气泡：当 loading 但最后一条消息还没有 content 时显示 */}
        {loading &&
          messages.length > 0 &&
          messages[messages.length - 1]?.content === "" && (
            <div className="flex gap-3 justify-start">
              ...
              <div className="rounded-2xl rounded-tl-sm bg-surface-2 border border-border/60 px-4 py-3 flex items-center gap-2">
                <span className="text-xs text-muted">AI 思考中</span>
                <span className="flex gap-1">
                  <span
                    className="w-1.5 h-1.5 rounded-full bg-pitch-bright animate-bounce"
      ...
      {messages.length > 0 && !loading && tasks.length === 0 && (
        <div
          className="shrink-0 px-4 pb-1 flex gap-2 overflow-x-auto"
          style={{ scrollbarWidth: "none" }}
        >
